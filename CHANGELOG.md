...
- [图片] 限制 sub2api 原生图生图在读取大参考图、Base64 编码与 JSON 提交期间的并发内存占用，避免应用在请求到达上游前 OOM 重启并把任务遗留为“待人工确认”。

- [Canvas] Agent 输入区随文字和参考图自动增高，图片/视频以行内标签插入光标位置并随文字换行；添加参考图入口移到底栏，长内容在输入区内滚动，保留发送及参数工具。

- [Canvas] 节点提示词及放大编辑改为原生富文本引用，编辑时同时保留图片/视频缩略图、高亮标签与准确光标；已有图片的未发送修改提示词随画布保存，关闭编辑框、切换节点及刷新后恢复，不覆盖原图生成提示词。

- [Canvas] 修复放大画布时节点输入框在上下两侧反复切换的闪烁；定位改用节点两侧的稳定空间，并按缩放比例换算高度限制。

- [Canvas] 蒙版编辑改用中性选区提示词，避免把蒙版透明语义误导为成图背景透明；保留独立蒙版、选区坐标及风格与光照约束。

## v0.0.24

- [Canvas] 修复生产公开 Session 未携带协议摘要，导致 Sunburst/sub2api 蒙版编辑仍被前端按本地 Alpha 蒙版硬裁切的问题；浏览器现在通过安全白名单的渠道协议和模型协议映射识别原生编辑，直接保留上游完整融合结果，不公开认证、路径或请求模板。

## v0.0.23

- [Canvas] 修复原生 sub2api 蒙版编辑仍向上游发送对象存储 URL、导致上游下载超时的问题；原图和蒙版现在由服务端安全读取后以内联 `data:image/...;base64` 提交，避免依赖上游回源。
- [Canvas] 蒙版编辑继承源图宽高比并保留用户选择的画质；竖版源图不再沿用旧节点的 `16:9`，避免 4K 结果被 `cover` 归一化裁切。原生 sub2api 结果继续完整保留，其他兼容协议仍使用本地蒙版保护未选区域。
- [测试] 协议矩阵改为验证 sub2api 内联原图与独立内联蒙版，同时保留安全出站对私网 URL 的拒绝。

## v0.0.22

- [Canvas] 修正 v0.0.21 的 sub2api 修图协议假设：原生图生图改用 `/images/edits`、`images[].image_url` 与独立 `mask.image_url`，蒙版不再混入场景参考；编辑提示保留原图风格、材质、光照与透视。原生结果不再被本地蒙版二次硬裁切，其他兼容渠道仍保留未选区域像素保护。
- [测试] 按实际 sub2api 源码解析规则建立 TCP 回归，并验证后台模型配置、系统代理、独立蒙版与单次计费；自动识别的旧 `image_urls` 接口保留原契约。

## v0.0.21

- [Canvas] 修复 sub2api 蒙版编辑只传原图、未把蒙版作为视觉输入且误用人物身份提示的问题；现在会把蒙版放在 `image_urls` 最后一项，并携带选区边界与中心坐标，使 Sunburst 等模型按指令在选区内生成目标内容，未选区域仍由本地像素合成保护。

## v0.0.20

- [Canvas] 蒙版局部编辑会持久化原图与蒙版，并在模型结果返回后只替换透明编辑区；刷新、人工检查状态和失败重试继续保护未选区域，兼容不严格执行蒙版的图片中转渠道。
